--------------------------------------------------------------------------------
-- Description: Vista per la TS in Full advertising_linear_management.package_sales
...
FROM `$source_project_daita.advertising_linear_management.package_sales`
WHERE id_package_sales IS NOT NULL
AND DATE(partition_date) = DATE((
      ...
      WHERE table_name="package_sales" AND partition_id!="__NULL__"
    ))
QUALIFY ROW_NUMBER() OVER (PARTITION BY id_package_sales ORDER BY _process_mapping_start_ts DESC)= 1